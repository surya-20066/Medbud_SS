...
const DoctorDashboard = () => {
  ...
  const [todayStats, setTodayStats] = useState({ total: 0, completed: 0, waiting: 0, revenue: 0 });
  ...
  const fetchDoctorData = useCallback(async (userId: string) => {
    ...
    setLoading(true);
    try {
      // Parallel fetch profile and doctor info
      ...
      // Parallel fetch tokens, appointments, booking requests, and records
      const [tokensRes, appointmentsRes, pendingRes, recordsRes] = await Promise.all([
        ...
        supabase
          ...
          .select(`*, profiles:patient_id (id, full_name, phone), tokens:token_id (token_number, token_date)`)
          .eq("doctor_id", doctorData.id)
          .order("created_at", { ascending: false })
      ]);
      ...
      setBookingRequests(pendingData);
      ...
      const revenue = tokensData.filter(t => t.status !== "cancelled").length * (doctorData.consultation_fee || 0);
      setTodayStats({ total: tokensData.length, completed, waiting, revenue });
    ...
    const checkSession = async () => {
      ...
      if (mounted) {
        if (currentSession) {
          ...
          fetchDoctorData(currentSession.user.id);
        } else {
          navigate("/auth");
        }
      }
    ...
    try {
      if (action === "accept") {
        ...
        if (appointment && appointment.patient_id) {
          // Create token for the appointment — use MAX to prevent duplicates
          const day = new Date(appointment.appointment_date).getDate().toString().padStart(2, '0');
          const dayPrefix = Number(day);
          // Determine the range of valid token numbers for this day (e.g. day 02 → 0200..0299..02999)
          const minRange = dayPrefix * 100; // e.g. 200 for day 02
          const maxRange = (dayPrefix + 1) * 100000; // generous upper bound
  ...
  // State to track if we just saved and should show the "add new" option
  ...
  return (
    <div className="min-h-screen bg-background">
                          ...
                          notifications.map((notif) => (
                            ...
                            >
                              <div className="flex gap-3">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${notif.type === 'booking' ? 'bg-blue-100 text-blue-600' :
              ...
              <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      >
                        ...
                        <p className="text-muted-foreground text-sm mb-4">Configure your profile details, UPI, and Working Hours</p>
                        <div className="flex items-center text-primary text-sm font-medium">
                    ...
                    <div className="grid grid-cols-1 gap-6 mt-6">
                            ...
                            <div key={token.id} className="p-4 border-b border-border last:border-0 flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${token.status === "in_progress"
                                  ...
                                  {formatTokenNumber(token.token_number, token.token_date)}
                                </div>
                                <div>
                      ...
                      <div className="bg-card rounded-xl p-6 border border-border shadow-soft">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-3xl font-bold text-blue-500">₹{todayStats.revenue}</p>
                    ...
                    <div className="bg-card rounded-xl border border-border overflow-hidden">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 p-4">
                        {tokens.filter(t => t.status === "completed").map((token) => (
                          ...
                          >
                            <p className="font-bold text-green-700 dark:text-green-400 group-hover:text-yellow-700 dark:group-hover:text-yellow-400">#{formatTokenNumber(token.token_number, token.token_date)}</p>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex gap-2">
                              ...
                              <div className="flex gap-2 flex-[2] w-full">
                                <Button
                                  size="sm"
                                  variant="outline"
                                  className="h-9 flex-1 text-xs font-semibold border-primary/20 hover:bg-primary/5 hover:border-primary/40"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setViewingRecordPatient(patient);
                                ...
                                <Button
                                  size="sm"
                                  className="h-9 flex-1 text-xs font-bold bg-primary text-white hover:bg-primary/90 shadow-sm"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setViewingRecordPatient(patient);
                          ...
                          <div key={record.id} className="p-4 border-b border-border last:border-0">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm text-muted-foreground">
                                {new Date(record.created_at).toLocaleDateString()} • Token #{formatTokenNumber(record.tokens?.token_number, record.tokens?.token_date)}
                              </span>
                            </div>
                            {record.diagnosis && (
                              <div className="mb-2">
                ...
                {viewMode === "settings" && doctorInfo && (
                  ...
                  >
                    ...
                    <div className="bg-card rounded-xl border border-border p-6 space-y-8 max-w-2xl">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-6 items-start">
                          <div className="w-24 h-24 rounded-full bg-muted border-4 border-background shadow-sm overflow-hidden flex items-center justify-center shrink-0">
                            ...
                            <div>
                              ...
                              <div className="flex gap-2">
                                <div className="relative">
                                  ...
                                  />
                                  <Label htmlFor="doc-take-pic" className="cursor-pointer bg-primary/10 hover:bg-primary/20 text-primary px-4 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors h-10 w-full sm:w-auto">
                                    <Camera className="w-4 h-4" /> Take a Pic
                                  </Label>
                                </div>
                                <div className="relative">
                                  <Input
                                  ...
                                  />
                                  <Label htmlFor="doc-gallery-upload" className="cursor-pointer bg-primary text-primary-foreground hover:bg-primary/90 px-4 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors h-10 w-full sm:w-auto">
                      ...
                      <div className="pt-6 border-t border-border space-y-6">
                        <h3 className="font-semibold text-lg flex items-center gap-2">
                          <div className="w-5 h-5 flex items-center justify-center rounded-full bg-primary/20 text-primary text-xs font-bold">₹</div>
                          Payment Details
                        </h3>
                        <div>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              // Also update auth user metadata for fallback
                        ...
                        {preponeRequests.map((req) => (
                          <div key={req.id} className={`bg-card rounded-xl border p-5 ${req.status === "pending" ? "border-orange-300 dark:border-orange-700" : "border-border"}`}>
                            <div className="flex items-start justify-between mb-3">
                              ...
                              <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${req.status === "pending" ? "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400" :
                          ...
                          return (
                            <div key={date}>
                              <div className="flex items-center gap-3 mb-3">
                                <div className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${isToday ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"
                                  }`}>
                                  {dayName}
                                </div>
                              ...
                              <div className="space-y-3">
                                {(dayAppointments as any[]).map((appt: any) => (
                                  <div
                                    key={appt.id}
                                    className={`bg-card rounded-xl border p-5 transition-all ${isToday ? "border-primary/30 shadow-md" : "border-border shadow-soft"
                                      }`}
                                  >
                                    <div className="flex items-start justify-between">
                                      <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-sm ${appt.status === "completed" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" :
                                      ...
                                      <div className="text-right">
                                        ...
                                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium mt-1 ${appt.status === "completed" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" :
                                          appt.status === "confirmed" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" :
                                            appt.status === "cancelled" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400" :
                                              "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400"
                                          }`}>
                                    ...
                                    {appt.payment_method && (
                                      <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                                        ...
                                        <span className={`px-2 py-0.5 rounded ${appt.payment_status === "completed" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-yellow-100 text-yellow-700"
                                          }`}>
      ...
      {showUpdateRecordModal && viewingRecordPatient && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => { setShowUpdateRecordModal(false); setJustSaved(false); setUpdatingRecordId(null); }} />
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
          ...
          >
            ...
            {justSaved ? (
              /* Success state after saving – offer to add a new record */
              <div className="p-6 flex flex-col items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                  <CheckCircle className="w-8 h-8 text-green-600" />
                </div>
                <p className="text-foreground font-medium text-center">Record has been saved successfully!</p>
                <p className="text-sm text-muted-foreground text-center">You can add another record for this patient or close this dialog.</p>
                <div className="flex gap-3 w-full mt-2">
                  ...
                  >
                    <FileText className="w-4 h-4 mr-2" /> Add New Record
                  </Button>
                  <Button variant="outline" className="flex-1" onClick={() => { setShowUpdateRecordModal(false); setJustSaved(false); setUpdatingRecordId(null); }}>
                    Done
                  </Button>
                </div>
              </div>
            ) : (
              /* Form state – scrollable content */
              <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar" style={{ maxHeight: "calc(90vh - 140px)" }}>
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-1">Diagnosis</label>
                    <Textarea
                      placeholder="Enter detailed diagnosis..."
                      value={updateRecordForm.diagnosis}
                      onChange={(e) => setUpdateRecordForm(prev => ({ ...prev, diagnosis: e.target.value }))}
                      rows={4}
                      className="rounded-xl bg-muted/30 border-border focus:ring-primary transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-1">Prescription</label>
                    <Textarea
                      placeholder="Medication names, dosage, and frequency..."
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      placeholder="Private notes for the patient..."
                  ...
                  {/* Image Upload */}
                  <div className="space-y-3">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-1">Clinical Attachments</label>
                    {updateRecordForm.attachments.length > 0 && (
                      <div className="grid grid-cols-3 gap-3 mb-4">
                        {updateRecordForm.attachments.map((img, i) => (
                          <div key={i} className="relative group aspect-square">
                            <img src={img} alt={`Attachment ${i + 1}`} className="rounded-xl border border-border w-full h-full object-cover shadow-sm" />
                            <button
                    ...
                    <label className="flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border-2 border-dashed border-border hover:border-primary hover:bg-primary/5 cursor-pointer transition-all group">
                      {uploadingImage ? (
                      ...
                      <div className="text-center">
                        <span className="text-sm font-semibold text-foreground block">
                          {uploadingImage ? "Uploading..." : "Click to upload clinical image"}
                        </span>
                ...
                <div className="flex gap-3 pt-6 border-t border-border sticky bottom-0 bg-card/95 backdrop-blur-sm -mx-6 px-6 pb-2">
                  <Button
                    className="flex-1 h-12 rounded-xl bg-primary text-white font-bold shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="text-center mb-6">
              <div className={`w-14 h-14 rounded-full mx-auto mb-4 flex items-center justify-center ${confirmToggle.newStatus === "completed"
              ...
              <p className="text-muted-foreground text-sm">
                ...
                <span className={`font-semibold ${confirmToggle.newStatus === "completed" ? "text-green-600" : "text-yellow-600"
                  }`}>
      ...
      {/* Lightbox for viewing images in-page */}
      {lightboxImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90" onClick={() => setLightboxImage(null)}>
          <button className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition-colors" onClick={() => setLightboxImage(null)}>
            <X className="w-6 h-6 text-white" />
          </button>
          <img src={lightboxImage} alt="Full size attachment" className="max-w-full max-h-[90vh] object-contain rounded-xl" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </div>
  );
